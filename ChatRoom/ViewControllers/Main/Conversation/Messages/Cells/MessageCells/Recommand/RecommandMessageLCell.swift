...
class RecommandMessageLCell<T: RecommandMessageCellVM>: MessageBaseLCell<T> {
    ...
    override func setupViews() {
        ...
        let edgeInsets = UIEdgeInsets(top: 8, left: 20, bottom: 8, right: 12)
        ...
        self.lblDescription.snp.makeConstraints {
            $0.top.equalTo(outlineContainerView.snp.bottom).offset(16)
            $0.leading.trailing.equalToSuperview()
        }

        self.btnAction.snp.makeConstraints {
            $0.top.equalTo(lblDescription.snp.bottom).offset(16)
...
private extension RecommandMessageLCell {
    func updateTextBackgroundImage(by order: MessageOrder) {
        let capInset = UIEdgeInsets(top: 26, left: 16, bottom: 12, right: 8)
        let bgImage = UIImage(named: "receive_bubble_pointer.9")?.resizableImage(withCapInsets: capInset, resizingMode: .stretch)
